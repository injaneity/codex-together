...
mod tests {
    ...
    #[tokio::test]
    async fn resume_thread_from_rollout_items_creates_local_thread() -> Result<()> {
        ...
        let resumed = app.resume_thread_from_rollout_items(history).await?;

        assert!(resumed.session_configured.rollout_path.is_some());
        Ok(())
    }
